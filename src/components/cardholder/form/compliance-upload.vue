...
<template>
    <div class="render-upload">
        <div class="render-upload-action" @click="triggerUpload" @drop.prevent="handleDrop" @dragover.prevent>
            <input
                v-if="!params.disabled && !isLoading"
                ref="fileInput"
                type="file" 
                ...
                @focus="handleFocus"
            />
            <Icon v-if="!isLoading && !modelValue?.url" type="ios-camera-outline" size="40" />
            <Spin v-if="isLoading" size="large" fix></Spin>
            <p v-if="!isLoading && !modelValue?.url">点击上传或将图片拖拽至此释放以上传</p>
            <div v-if="modelValue && modelValue.url && !isLoading" class="preview">
...
// 触发文件选择
const triggerUpload = () => {
    if(props.params.disabled || isLoading.value) return false;
...
const uploadFile = async (file) => {
    ...
    try {
        ...
        const res = await cardApi.uploadComplianceFile(formData)
        if (res) {
            const uploadResult = {
                url: res.fileFullUrl,
                fileId: res.fileId
