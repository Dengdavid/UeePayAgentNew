<template>
  <div class="UploadImg">
    <div class="UploadImgBox">
      <FormUpload :format="format" :max-size="2048" UploadType="img" :width="width" :height="height" v-bind="$attrs"
          ...
          <template v-if="url">
            <div class="action" @click.stop>
              <Tooltip content="预览" placement="top">
                <Icon type="md-eye" @click="openImagePreview(url)" />
              </Tooltip>
              <Tooltip content="删除" placement="top">
                <Icon type="md-trash" @click="handleChange" />
              </Tooltip>
            </div>
            <Image :src="url" :fit="fit" :width="sizeNum(width, '100%')" :height="sizeNum(height, width)" />
          </template>
          <div class="imgBox" v-else>
            <template v-if="document">
              <Image :src="getImageUrl(document)" fit="cover" />
            </template>
    ...
    <FormPup v-model="visiblePup" width="800" v-model:loading="loading" title="示例文件" :actions="pupAction">
      <div class="example-preview-list">
        <FormList :data="pupList" isBorder isCenter :height="180" :tag="(item, index) => `示例${index + 1}`">
...
const format = ref(['jpg', 'jpeg', 'png', 'gif', 'bmp', 'webp'])
const urlList = ref([])
const pupList = ref([])
...
const isImageFile = (fileNamesStr) => {
  ...
  return fileNames.every(fileName => {
    ...
    if (!ext) return false;
    // 检查是否在白名单中
    return format.value.includes(ext);
  });
}
