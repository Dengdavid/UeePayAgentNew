<template>
   <div class="cardTag list-b-2"
    :class="{
      ...
      <Icon :type="checked ? 'ios-radio-button-on' : 'ios-radio-button-off'" class="radio-icon"/>
      <div class="cardTag-wrap">
        ...
        <div class="cardTag-sub">
          ...
          <span>{{ $t('cardTag.remainingPrefix') }}<span class="count">{{ item.allow_create_count < 0 ? '∞' : item.allow_create_count }}</span>{{ getCardUnit(item.allow_create_count) }}</span>
        </div>
      </div>
  </div>
</template>
...
const getCardUnit = (value) => t('cardTag.cardUnit', {
  count: Number(value) < 0 ? 2 : Number(value) || 0,
})
