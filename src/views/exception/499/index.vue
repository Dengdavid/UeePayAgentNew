  ...
  <div class="not-found">
    ...
    <div class="content">
      ...
      <div
        ...
        "
      ></div>
      <p class="description">{{ $t('exception.regionRestricted') }}</p>
      <!-- 返回按钮 -->
      <Button
        type="primary"
        class="mt-20"
        :disabled="dateTime > 0"
        @click="handleBack"
        >{{ dateTime > 0 ? $t('exception.acknowledgedCountdown', { seconds: dateTime }) : $t('exception.acknowledged') }}</Button
      >
    </div>
  </div>
</template>
...
const dateTime = ref(60 * 60 * 24 * 7)
const dateTimeRef = ref(null)
const setTime = () => {
  dateTimeRef.value = setInterval(() => {
    dateTime.value--
    if (dateTime.value <= 0) {
      clearInterval(dateTimeRef.value)
    }
  }, 1000)
}
const handleBack = () => {
