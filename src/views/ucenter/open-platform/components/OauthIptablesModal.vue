...
const setIptables=(ips)=>{
  ...
  if (duplicateIp) {
      message(t('openPlatform.ipWhitelist.duplicate', { ip: duplicateIp }),'error');
      return false;
  }
...
const pup = reactive({
  status:false,
  title: t('openPlatform.ipWhitelist.title'),
  width:400,
  form:{
    iptables:'',
  },
  actions:[
    {
      ...
      click: (pup) => {
        ...
        postApi('/user/setOauthIpTables',{
          oauth_iptables:finalIptables,
        }).then((res) =>{
          message(t('openPlatform.ipWhitelist.success'))
          close()
          emits('success')
        }).catch((err) =>{
          message(err?.msg || t('openPlatform.ipWhitelist.failed'),'error')
        }).finally(()=>{
          pup.loading=false
        })
      }
